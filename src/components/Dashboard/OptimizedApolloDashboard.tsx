...
const LazyResultsTableAndModals = lazy(() => import('../Resultssection/OptimizedResultsTableAndModals'));
...
const OptimizedApolloDashboard: React.FC<DashboardProps> = memo(({ 
  isDarkMode, 
  showWebsiteFilters, 
  ...
  // Removed artificial delay and showResults state for immediate loading
  ...
  // Preload the results component immediately
  useEffect(() => {
    const preloadResults = async () => {
      try {
        await import('../Resultssection/OptimizedResultsTableAndModals');
      } catch (error) {
        ...
        {searchFiltersVisible && (
          <div className="w-64 flex-shrink-0 border-r border-gray-200 dark:border-gray-700">
            <SearchAndFilterSection
              ...
              isVisible={searchFiltersVisible}
              toggleVisibility={toggleSearchFiltersVisibility}
            />
          </div>
        )}
...
OptimizedApolloDashboard.displayName = 'OptimizedApolloDashboard';

export default OptimizedApolloDashboard;